...
def run_episode_with_sampling():
    ...
    try:
        ...
        for strategy_name, strategy_type in strategies:
            ...
            for step in range(2000):  # 限制2000步
                ...
                if info_dict and len(info_dict) > 0:
                    ...
                    if game_info.get("flag_get", False):
                        print(f"🏁 通关成功! 步数: {steps}, 奖励: {total_reward:.1f}, 位置: {x_pos}")
                        break
                
                # 每200步输出进度
                if (step + 1) % 200 == 0:
                    print(f"  步数: {step+1:4d} | 最远位置: {max_x_pos:4d} | 累计奖励: {total_reward:7.1f}")
            ...
            print(f"结果 - 奖励: {total_reward:7.1f} | 最远: {max_x_pos:4d} | 步数: {steps:4d}")
            
        # 总结对比
        print(f"\n=== 策略对比总结 ===")
        for result in results:
            success_mark = "✓" if result["success"] else "✗"
            print(f"{success_mark} {result['strategy']:8s}: 奖励={result['reward']:7.1f} 最远={result['max_x_pos']:4d} 进度={result['progress']:4d}")
