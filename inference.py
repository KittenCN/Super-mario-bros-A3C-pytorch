...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="推理 Super Mario Bros A3C 模型 (支持目录自动解析 / 前缀自适应加载)")
    parser.add_argument(
        "--checkpoint", 
        type=str, 
        required=True, 
    ...
    parser.add_argument(
        "--max-missing-keys", type=int, default=20, help="控制打印缺失/多余权重键的最大数量 (默认 20)"
    )
    parser.add_argument(
        "--issues-log-dir", type=str, default=".", help="保存 state_dict 加载差异日志的目录 (默认当前)"
    )
...
def create_inference_env(metadata: dict, render: bool = False, record_video: bool = False, full_schedule: bool = False):
    """根据模型元数据创建推理环境。
    ...
    if full_schedule:
        ...
        schedule = [(int(w), int(s)) for (w, s) in raw_sched] if raw_sched else [(metadata["world"], metadata["stage"])]
    else:
...
def load_model(checkpoint_path: Path, metadata: dict, device: torch.device, max_missing: int, issues_log_dir: Path) -> MarioActorCritic:
    ...
    if issues:
        ...
        more = "" if len(issues) <= max_missing else f" ... (+{len(issues)-max_missing} more)"
        ...
        try:
            ...
            print(f"[inference] 差异已写入: {(issues_log_dir / ISSUES_LOG_NAME).as_posix()}")
...
def main():
    ...
    try:
        ...
        for episode in range(args.episodes):
            ...
            print(f"  结果: {status}")
            print(f"  奖励: {reward:8.1f} | 步数: {steps:5d} | 时长: {episode_duration:6.2f}s | 速率: {steps_per_sec:6.1f} steps/s")
            print("-" * 80)
        ...
        if total_env_time > 0:
            print(f"  平均推理速率: {total_env_steps/total_env_time:6.1f} steps/s (基于无渲染计时)")
