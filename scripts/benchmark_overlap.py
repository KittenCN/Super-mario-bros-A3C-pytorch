...
def run_case(
    ...
    metrics_path = (
        Path("benchmarks")
        / f"metrics_{'ol' if overlap else 'sync'}_{num_envs}e_{rollout}r_{ts}.jsonl"
    )
    ...
    return {
        ...
        "replay_fill_final": (
            round(use[-1].get("replay_fill_rate", 0.0), 4) if use else 0.0
        ),
        "gpu_util_mean_window": (
            round(use[-1].get("gpu_util_mean_window", 0.0), 2) if use else 0.0
        ),
...
def main():
    ...
    ap.add_argument(
        "--warmup-fraction", type=float, default=0.3, help="丢弃前比例作为 warmup"
    )
    ap.add_argument("--no-sync-only", action="store_true", help="仅运行 overlap 模式")
    ap.add_argument(
        "--extra", type=str, nargs="*", default=[], help="透传 train.py 其它参数"
    )
    args = ap.parse_args()
    cases = []
