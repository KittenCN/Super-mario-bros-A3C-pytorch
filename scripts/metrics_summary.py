...
DEFAULT_FIELDS = [
    "update","global_step","avg_return","recent_return_p50","recent_return_p90","recent_return_p99",
    "env_steps_per_sec","updates_per_sec","replay_fill_rate","replay_last_unique_ratio","replay_avg_unique_ratio",
    "replay_priority_mean","replay_priority_p90","gpu_util_mean_window","loss_total","loss_value","loss_policy"
]
...
def main():
    ...
    ap.add_argument("--fields", type=str, default="", help="逗号分隔聚合字段; 留空用默认列表")
    ap.add_argument("--parquet", action="store_true", help="导出 Parquet (需要 pandas+pyarrow)")
    args=ap.parse_args()
    ...
    fields=[f.strip() for f in args.fields.split(",") if f.strip()] if args.fields else DEFAULT_FIELDS
    ...
    # 控制台摘要
    key_print=[
        "avg_return_last","recent_return_p90_last","env_steps_per_sec_mean","updates_per_sec_mean",
        "replay_fill_rate_last","replay_avg_unique_ratio_last","gpu_util_mean_window_last"
    ]
