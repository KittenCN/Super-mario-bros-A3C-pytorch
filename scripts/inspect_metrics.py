...
def parse_args():
    ...
    ap.add_argument('--tail', type=int, default=100, help='分析最近 N 条拥有 update 字段的记录')
...
def main():
    ...
    with p.open('r', encoding='utf-8') as fp:
        for line in fp:
            ...
            if 'update' not in obj:
                continue
            updates.append(obj)
            if first_positive is None and int(obj.get('env_distance_delta_sum',0) or 0) > 0:
                first_positive = obj['update']
    if not updates:
        print('[inspect] 没有可解析的 update 指标行')
        return 0
    recent = updates[-args.tail:]
    pos_ratios=[float(o.get('env_positive_dx_ratio',0.0) or 0.0) for o in recent if 'env_positive_dx_ratio' in o]
    ...
    report = {
        ...
        'first_positive_distance_update': first_positive if first_positive is not None else -1,
