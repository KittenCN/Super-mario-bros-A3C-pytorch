...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--checkpoint", type=str, required=True, help=".pt 文件或包含 checkpoint 的目录")
    ...
    p.add_argument("--full-schedule", action="store_true", help="使用 metadata 中完整 stage_schedule")
    p.add_argument("--issues-log-dir", type=str, default=".", help="保存差异日志目录")
    p.add_argument("--max-missing-keys", type=int, default=20, help="控制打印缺失/多余键数量")
    return p.parse_args()
...
def main():
    ...
    if issues:
        ...
        more = "" if len(issues) <= args.max_missing_keys else f" ... (+{len(issues)-args.max_missing_keys} more)"
    ...
    try:
        for ep in range(args.episodes):
            ...
            for step in range(args.max_steps):
                ...
                # reward shape 兼容
                try:
                    r_scalar = float(rew_np[0]) if hasattr(rew_np, '__len__') else float(rew_np)
                except Exception:
                ...
                if info_item:
                    ...
                    if x_val is not None:
                        ...
                        try:
                            ...
                            if (step + 1) % 100 == 0:
                                print(f"  step={step+1:4d} x={xv_int:4d} reward={total_reward:7.1f}")
                        except Exception:
                            if (step + 1) % 100 == 0:
                                print(f"  step={step+1:4d} x=? raw={xv} reward={total_reward:7.1f}")
                    else:
                        if (step + 1) % 100 == 0:
                ...
                if term_flag or trunc_flag:
                    print(f"  ❌ 结束 steps={step+1} max_x={max_x} reward={total_reward:.1f}")
                    break
            dur = time.time() - start
            sps = (step + 1) / dur if dur > 0 else 0
            print(f"[quick] Episode 完成: steps={step+1} reward={total_reward:.1f} max_x={max_x} speed={sps:6.1f} steps/s")
            time.sleep(0.5)
    finally:
        env.close()
        print("\n[quick] 推理完成")
