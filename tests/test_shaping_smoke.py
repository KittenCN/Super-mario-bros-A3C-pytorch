import os, json, subprocess, sys, time, tempfile, pathlib
...
def run_short():
    ...
    dry = subprocess.run(['bash', str(SCRIPT), '--dry-run'], env=env, capture_output=True, text=True)
    if dry.returncode != 0:
    ...
    if not (distance_nonzero or shaping_nonzero):
        tail = ''.join(lines[-50:])
        raise AssertionError('No non-zero distance or shaping observed in smoke run. Stdout tail:\n'+tail)
