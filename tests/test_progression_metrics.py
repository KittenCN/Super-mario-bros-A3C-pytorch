...
def run_short_train(tmpdir: pathlib.Path):
    ...
    cmd = [
        ...
        '--rollout-steps','16',
        '--log-interval','1',
        '--early-shaping-window','10',
        '--secondary-script-threshold','5',
        '--secondary-script-frames','20',
        '--milestone-interval','0',
        '--episode-timeout-steps','120',
        '--scripted-forward-frames','32',
    ]
    env = os.environ.copy()
...
def test_progression_metrics_presence():
    with tempfile.TemporaryDirectory() as td:
        ...
        for o in updates:
            ...
            if val > max_val:
                max_val = val
        assert max_val >= updates[-1].get('env_distance_delta_sum',0), 'max 距离统计异常'
        # 至少一个推进/自救机制指标出现
        mechanism_present = any(k in last for k in [
            'auto_bootstrap_triggered','secondary_script_triggered','milestone_count'
        ])
